...
public class MailService {
	...
	public MimeMessage createEmailForm(String email) 
		...
		String setFrom = "beckgo34@naver.com"; // mail setting에 설정한 메일 주소(보내는 사람)
	...
	// Thymeleaf를 이요한 context(HTML, 메일 화면)를 설정하는 메소드
	...
	public String sendEmail(MemberDto member, HttpSession session) {
		...
		try {
			...
			if(email.equals(member.getM_email())) {
				emailFrom = createEmailForm(email);
